...
def main():
    ...
    comparison_img_name = 'ComparisonIMG2.jpg'  # 根据实际文件名调整
    ...
    for i in tqdm(range(1, total_images + 1), desc="Processing Images"):
        ...
        if not os.path.exists(image_path):
            ...
            results.append({
                ...
                'CurrentImage_Score': assessment_score,
                'CurrentImage_Final_Score':assessment_REscore
                    ...
                    "parts": [
                        {
                            "text": (
                                f"Please assess the quality of the following image comprehensively. "
                                f"This image is named {image_name}. When evaluating, consider both objective technical factors (such as sharpness, contrast, color accuracy, etc.) and subjective user experience factors (such as aesthetic appeal, emotional impact, etc.). "
                                f"Identify the key aspects that determine the image's quality and provide a detailed assessment for each aspect, ensuring that acceptable quality images are not unduly penalized."
                            )
                        },
                        {
                            "inline_data": {
        ...
        # ------------------------------------
        # 第三轮对话：根据ComparisonIMG分数计算当前图片分数
        # ------------------------------------        
        for attempt in range(1, max_retries + 1):
            try:
                # 构建用户消息，包含文本
                user_message_score = {
                    "parts": [
                        {
                            "text": (
                                f"Based on our previous analysis of the two images, ComparisonIMG has been given a quality score of 54. The evaluation criteria include, but are not limited to, the following aspects:{initial_assistant_response}"
                                f"Please provide a quality score for {image_name} based on the above criteria, in conjunction with the detailed analysis of the two images previously conducted. Ensure that the scoring is consistent with the standards used for ComparisonIMG, and briefly explain the basis of your assessment after scoring. "
                            )
                        }
                    ]
                }
                ...
                # 发送消息并接收响应
                response_score = chat.send_message(user_message_score)
                assistant_response_score = response_score.text.strip()

                # 打印和记录评估结果
                assessment_score = assistant_response_score
                #print(f"Assessment for {image_name}'s score: {assessment_score}")
                break
                ...
                if attempt < max_retries:
                    ...
                    print(f"Failed to process {image_name}'s score after {max_retries} attempts.")            
        ...
        for attempt in range(1, max_retries + 1):
            try:
                # 构建用户消息，包含文本
                user_message_REscore = {
                    "parts": [
                        {
                            "text": (
                                f"The initial quality score for ComparisonIMG is 54 based on our previous analysis. However, to ensure a more accurate and balanced evaluation, I would like you to reassess the {image_name} score by considering additional factors.\n"
                                f"Please follow these steps:\n"
                                f"1. **Review Previous Analysis**: Summarize the key factors previously used to assess both ComparisonIMG and {image_name}, including both objective technical aspects and subjective user experience aspects.\n"
                                f"2. **Identify Additional Factors**: Propose any other factors that might influence image quality assessment that were not previously considered, ensuring to include subjective factors if applicable. Analyze each new factor in detail.\n"
                                f"3. **Integrate New Factors with Previous Analysis**: Combine the newly identified factors with the existing ones to form a comprehensive evaluation framework that balances objective and subjective assessments.\n"
                                f"4. **Re-evaluate {image_name}**: Using the integrated framework, provide a revised quality score for {image_name}. Ensure that the scoring is consistent and justifiable based on both the original and newly introduced factors, maintaining a fair evaluation that avoids overly harsh scores for acceptable images.\n"
                                f"5. **Explain the Reasoning**: Provide a step-by-step explanation of how each factor influenced the revised score to ensure transparency and clarity."
                            )
                        }
                    ]
                }
                ...
                # 发送消息并接收响应
                response_REscore = chat.send_message(user_message_REscore)
                assistant_response_REscore = response_REscore.text.strip()

                # 打印和记录评估结果
                assessment_REscore = assistant_response_REscore
                #print(f"Assessment for {image_name}'s final score: {assessment_REscore}")
                break
                ...
                if attempt < max_retries:
                    ...
                    print(f"Failed to process {image_name}'s final score after {max_retries} attempts.")
    ...
    # 保存DataFrame到Excel文件
    output_excel = 'Score(final)2.xlsx'
    try:
