...
def main():
    ...
    for i in tqdm(range(1, total_images + 1), desc="Processing Images"):
        ...
        if not os.path.exists(image_path):
            ...
            results.append({'Image': image_name, 'Assessment_ComparisonIMG': None, 'Assessment_CurrentImage': assessment})
            continue
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # 创建带有结构化内容的用户部分（发送 ComparisonIMG）
                user_content_comparison = {
                    "parts": [
                        {
                            "text": (
                                f"Please assess the quality of the following image comprehensively. This image is named ComparisonIMG. Identify the key aspects that determine the image's quality and provide a detailed assessment for each aspect."
                            )
                        },
                        {
                            "inline_data": {
                ...
                # 合并内容
                temp_content_comparison = {
                    "parts": initial_content["parts"] + user_content_comparison["parts"]
                }

                # 调用API生成内容
                response_comparison = model.generate_content(temp_content_comparison)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # 创建带有结构化内容的用户部分（发送当前图片）
                user_content_current = {
                    "parts": [
                        {
                            "text": (
                                f"Please assess the quality of the following image comprehensively. This image is named {image_name}. Identify the key aspects that determine the image's quality and provide a detailed assessment for each aspect."
                            )
                        },
                        {
                            "inline_data": {
                ...
                # 合并内容
                temp_content_current = {
                    "parts": initial_content["parts"] + user_content_current["parts"]
                }

                # 调用API生成内容
                response_current = model.generate_content(temp_content_current)
    ...
    # 保存DataFrame到Excel文件
    output_excel = 'ChatFactors1.xlsx'
    try:
