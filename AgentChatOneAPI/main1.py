...
def main():
    ...
    for batch_start in range(1, total_images + 1, 50):
        ...
        for i in tqdm(range(batch_start, batch_end + 1), desc=f"Processing Images {batch_start}-{batch_end}"):
            ...
            try:
                # Initialize chat sessions using OpenAI
                responder_chat = initialize_chat(responder_system_message, role='system')
                asker_chat = initialize_chat(asker_system_message, role='system')
                judge_chat = initialize_chat(judge_system_message, role='system')
            except Exception as e:
            ...
            for cycle in range(1, num_cycles + 1):
                ...
                judge_evaluate_asker_prompt = (
                    ...
                    f"Respond with 'on-topic' or 'off-topic'. If 'off-topic', provide a gentle reminder to the Inquisitive Analyst to focus on image quality assessment. Your reminder should start with \"Remind that:\"\n\n"
                    f"Asker's Question:\n{asker_question}"
                )
                ...
                judge_evaluate_responder_prompt = (
                    ...
                    f"Respond with 'on-topic' or 'off-topic'. If 'off-topic', provide a gentle reminder to the Image Analysis Expert to focus on image quality assessment. Your reminder should start with \"Remind that:\"\n\n"
                    f"Responder's Response:\n{responder_response}"
                )
